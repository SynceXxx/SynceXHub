...
local premiumStatus = Player.MembershipType == Enum.MembershipType.Premium
...
profileGroup:Paragraph({
    ...
    Desc = "User ID: " .. userId .. "\nAccount Age: " .. accountAge .. " days" .. (premiumStatus and "\n⭐ Premium Member" or ""),
    Image = "user-circle",
    ImageSize = 28,
    Buttons = {
        {
...
-- Real-Time Clock
local clockGroup = home:Group({})

local clockSection = clockGroup:Section({
    Title = "⏰ Loading...",
    TextSize = 22,
    FontWeight = Enum.FontWeight.Bold,
})

local dateSection = clockGroup:Section({
    Title = "Loading date...",
    TextSize = 15,
...
task.spawn(function()
    while true do
        ...
        pcall(function()
            clockSection.Title = "🕐 " .. hour .. ":" .. min .. ":" .. sec
            dateSection.Title = date
        end)
...
local perfGroup = home:Group({})

-- FPS Counter
local fpsSection = perfGroup:Section({
...
task.spawn(function()
    ...
    RunService.RenderStepped:Connect(function()
        ...
        if tick() - lastUpdate >= 1 then
            ...
            local indicator = fps >= 60 and "🟢" or (fps >= 30 and "🟡" or "🔴")
            
            pcall(function()
                fpsSection.Title = indicator .. " FPS: " .. tostring(fps) .. " / 60"
            end)
        end
    end)
end)
...
-- Ping Counter
local pingSection = perfGroup:Section({
...
task.spawn(function()
    while true do
        ...
        local indicator = ping < 100 and "🟢" or (ping < 200 and "🟡" or "🔴")
        
        pcall(function()
            pingSection.Title = indicator .. " Ping: " .. ping .. " ms"
        end)
...
-- ============================================================
-- ABOUT SYNCEHUB SECTION
-- ============================================================
home:Section({
    Title = "About SynceHub",
    Icon = "info",
    TextSize = 20,
    FontWeight = Enum.FontWeight.SemiBold,
})
...
home:Paragraph({
    Title = "SynceHub Universal Script Hub",
    Desc = "About SynceHub which will always be updated and always provide the best for the future",
    Image = "rbxassetid://114915707934715",
    ImageSize = 32,
})
