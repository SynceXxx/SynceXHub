...
-- ============================================================
    ...
    -- ============================================================
-- SYNCEHUB - ANIMATION TAB (NEW VERSION)
-- ============================================================

local AnimationTab = {}

function AnimationTab.Create(Window, Reg, WindUI, LocalPlayer)
    ...
    function AnimationSystem.Apply(player, animationSet)
        ...
        -- Function to change animation ID
        ...
        if animationSet.walk then
            ChangeAnim("walk", animationSet.walk)
        end
        
        if animationSet.run then
            ChangeAnim("run", animationSet.run)
        end
        
        if animationSet.jump then
            ChangeAnim("jump", animationSet.jump)
        end
        
        if animationSet.fall then
            ChangeAnim("fall", animationSet.fall)
        end
        
        if animationSet.swim then
            ChangeAnim("swim", animationSet.swim)
        end
        
        if animationSet.climb then
            ChangeAnim("climb", animationSet.climb)
        end
    ...
    -- Reset to default Roblox animations
    ...
    function AnimationSystem.GetPresets()
        ...
        table.sort(presets) -- Sort alphabetically
        return presets
    end
    ...
    animation:Section({
        Title = "Apply pre-made animation sets to your character",
        TextSize = 14,
    ...
    -- ============================================================
    local currentPresetLabel = animation:Section({
    ...
    for i, presetName in ipairs(presets) do
        animation:Button({
            ...
            Callback = function()
                ...
                if success then
                    ...
                    currentPresetLabel:SetTitle("Current: " .. presetName)
                    
                    WindUI:Notify({
                        Title = "Animation Applied",
                        Content = presetName .. " animation set applied!",
                        Duration = 2,
                        Icon = "check",
                    })
                else
        ...
        -- Add space between buttons (except last one)
        if i < #presets then
            animation:Space()
        end
    end
    ...
    animation:Button({
        ...
        Callback = function()
            ...
            if success then
                currentPresetLabel:SetTitle("Current: Default")
    ...
    animation:Section({
        Title = "Enter custom animation IDs for each animation type",
        TextSize = 14,
        TextTransparency = 0.4,
    })
    
    -- Storage for custom IDs
    local customIds = {
        idle = nil,
        walk = nil,
        run = nil,
    ...
    -- Input fields for each animation type
    animation:Input({
    ...
    -- Apply custom animations button
    animation:Button({
        Title = "Apply Custom Animations",
        Icon = "check",
        Callback = function()
            -- Check if at least one custom ID is provided
            ...
            if success then
                currentPresetLabel:SetTitle("Current: Custom")
    ...
    animation:Paragraph({
        Title = "Finding Custom Animation IDs",
        Desc = "1. Go to Roblox website\n2. Search for animations in the catalog\n3. Open an animation page\n4. Copy the number from the URL\n\nExample URL: roblox.com/library/507766388/\nAnimation ID: 507766388\n\nYou can also find animations on the Roblox Creator Store or use animation IDs from popular Roblox games.",
    })
    
    animation:Paragraph({
        Title = "Tips & Notes",
        Desc = "• Preset animations are pre-configured sets that work well together\n• Custom animations let you mix and match individual animation IDs\n• Not all animations may work (depends on creator permissions)\n• You can reset to default Roblox animations anytime\n• Some games may override your animations",
    })
