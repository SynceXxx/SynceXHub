...
function TabContent.CreateTabs(Window, Reg, WindUI, SynceHubConfig, SmartLoadConfig, BaseFolder, ElementRegistry)
    ...
    -- STATS & INFO SECTION
    -- ============================================================
    local stats = choptree:Section({
        Title = "Stats & Information",
        Icon = "bar-chart",
        TextSize = 19,
    })
    
    local StatsGroup = stats:Group({})
    
    local CoinsText = StatsGroup:Paragraph({
        Title = "Coins",
        Desc = "Loading...",
    })
    
    local PrestigeText = StatsGroup:Paragraph({
        Title = "Prestiges",
        Desc = "Loading...",
    })
    
    local WaterText = StatsGroup:Paragraph({
        Title = "Highest Water Level",
        Desc = "Loading...",
    })
    
    local PlotText = StatsGroup:Paragraph({
        Title = "Plot Status",
        Desc = "Loading...",
    })
    ...
    task.spawn(function()
        while task.wait(1) do
            if not ChopTreeSystem.State.Running then break end
            
            pcall(function()
                local coins = ChopTreeSystem.GetCoinsValue()
                CoinsText:Set({
                    Title = "Coins",
                    Desc = coins,
                })
                
                local prestiges = ChopTreeSystem.GetPrestigesValue()
                PrestigeText:Set({
                    Title = "Prestiges",
                    Desc = tostring(prestiges),
                })
                
                local waterLevel = ChopTreeSystem.GetHighestWaterLevel()
                local waterReady = waterLevel >= ChopTreeSystem.Config.WaterLevelThreshold and "Ready" or "Not Ready"
                WaterText:Set({
                    Title = "Highest Water Level",
                    Desc = tostring(waterLevel) .. waterReady,
                })
                
                local plot = ChopTreeSystem.GetPlayerPlot()
                PlotText:Set({
                    Title = "Plot Status",
                    Desc = plot and "Plot Found" or "No Plot",
                })
            end)
        end
    end)
