-- ========================================
-- SYNCE EMOTES - REDESIGNED UI (PART 1/5)
-- Modern Dark Theme - FIXED STROKE
-- ========================================
...
function scale(axis, value)
    ...
    local scaleFactor = isMobile and 2.5 or 2
...
local savedEmotes = {}
local SAVE_FILE = "SynceCatalog.json"
...
-- FIXED: Stroke lebih tipis
local function createStroke(parent, color, thickness)
    local stroke = Instance.new("UIStroke")
    stroke.Color = color or Color3.fromRGB(60, 60, 70)
    stroke.Thickness = thickness or 0.5 -- Dikecilkan dari 1 jadi 0.5
    stroke.Transparency = 0.7 -- Lebih transparan
...
createStroke(mainContainer, Color3.fromRGB(70, 70, 85), 1) -- Tetap 1 untuk main container

-- FIXED: Shadow lebih kecil dan halus
local shadowFrame = Instance.new("Frame")
shadowFrame.Size = UDim2.new(1, 20, 1, 20) -- Dikecilkan dari 30 jadi 20
shadowFrame.Position = UDim2.new(0, -10, 0, -10) -- Dikecilkan dari -15 jadi -10
shadowFrame.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
shadowFrame.BackgroundTransparency = 0.8 -- Lebih transparan dari 0.7
shadowFrame.ZIndex = -1
shadowFrame.Parent = mainContainer
createCorner(shadowFrame, 25)
...
-- ========================================
-- SYNCE CATALOG - MULTI CATEGORY (PART 2/5)
-- Header, Tabs, Controls + CATEGORY DROPDOWN
-- ========================================
...
-- ========================================
-- CATALOG FRAME WITH CATEGORY DROPDOWN
-- ========================================
...
categoryBtn.Size = UDim2.new(0.35, 0, 0, scale("Y", 36))
categoryBtn.Position = UDim2.new(0, 0, 0, 0)
categoryBtn.BackgroundColor3 = Color3.fromRGB(60, 50, 120)
categoryBtn.Text = "🎭 Emotes ▼"
categoryBtn.Font = Enum.Font.GothamBold
categoryBtn.TextSize = scale("Y", 13)
categoryBtn.TextColor3 = Color3.new(1, 1, 1)
categoryBtn.Parent = catalogFrame
createCorner(categoryBtn, 10)
addHoverEffect(categoryBtn, Color3.fromRGB(80, 70, 140), Color3.fromRGB(60, 50, 120))

-- Category dropdown menu
local categoryDropdown = Instance.new("ScrollingFrame")
categoryDropdown.Size = UDim2.new(0, scale("X", 180), 0, scale("Y", 200))
categoryDropdown.Position = UDim2.new(0, 0, 0, scale("Y", 40))
categoryDropdown.BackgroundColor3 = Color3.fromRGB(30, 30, 38)
categoryDropdown.ScrollBarThickness = 4
categoryDropdown.Visible = false
categoryDropdown.ZIndex = 10
categoryDropdown.Parent = catalogFrame
createCorner(categoryDropdown, 10)

local categoryLayout = Instance.new("UIListLayout", categoryDropdown)
categoryLayout.Padding = UDim.new(0, 2)

-- Search Box
local searchContainer = Instance.new("Frame")
searchContainer.Size = UDim2.new(0.3, 0, 0, scale("Y", 36))
searchContainer.Position = UDim2.new(0.37, scale("X", 2), 0, 0)
...
refreshBtn.Size = UDim2.new(0.15, -scale("X", 4), 0, scale("Y", 36))
refreshBtn.Position = UDim2.new(0.69, scale("X", 2), 0, 0)
refreshBtn.BackgroundColor3 = Color3.fromRGB(40, 120, 220)
refreshBtn.Text = "🔄"
refreshBtn.Font = Enum.Font.GothamBold
refreshBtn.TextSize = scale("Y", 16)
...
sortBtn.Size = UDim2.new(0.16, -scale("X", 6), 0, scale("Y", 36))
sortBtn.Position = UDim2.new(0.84, scale("X", 4), 0, 0)
sortBtn.BackgroundColor3 = Color3.fromRGB(60, 50, 120)
sortBtn.Text = "📊"
sortBtn.Font = Enum.Font.GothamBold
sortBtn.TextSize = scale("Y", 16)
...
savedEmptyLabel.Text = "💫 No saved items yet"
...
-- ========================================
-- SYNCE EMOTES - REDESIGNED UI (PART 3/5)
...
-- ========================================
-- SYNCE CATALOG - MULTI CATEGORY (PART 4A/5) - FULLY FIXED
-- All Bugs Fixed: getanimid, category dropdown, wear system
-- ========================================
...
local CATEGORIES = {
    {Name = "🎭 Emotes", AssetType = 61, CanPlay = true, CanEquip = false},
    {Name = "🎩 Hats", AssetType = 8, CanPlay = false, CanEquip = true},
    {Name = "👕 Shirts", AssetType = 11, CanPlay = false, CanEquip = true},
    {Name = "👖 Pants", AssetType = 12, CanPlay = false, CanEquip = true},
    {Name = "👔 T-Shirts", AssetType = 2, CanPlay = false, CanEquip = true},
    {Name = "😊 Faces", AssetType = 18, CanPlay = false, CanEquip = true},
    {Name = "💇 Hair", AssetType = 41, CanPlay = false, CanEquip = true},
    {Name = "🎒 Back", AssetType = 44, CanPlay = false, CanEquip = true},
    {Name = "👓 Face Acc", AssetType = 42, CanPlay = false, CanEquip = true}
}
...
local function GetCatalogDataFromWeb()
    local currentCat = getCurrentCategory()
    print("[DEBUG] Fetching category:", currentCat.Name)
    
    local url_parts = {
        CATALOG_URL .. "?model.assetTypeIds=" .. currentCat.AssetType,
    ...
    print("[SUCCESS] Loaded", #data_table.data, "items. Total:", #ITEM_CACHE)
...
-- DECLARE scroll GLOBALLY
_G.SYNCE_SCROLL = nil

-- ========================================
-- FIXED: Safe getanimid function
-- ========================================
local function safeGetAnimId(assetId)
    local success, result = pcall(function()
...
-- ========================================
-- FIXED: createCard Function
-- ========================================

local function createCard(item, scrollParent)
    local currentCat = getCurrentCategory()
    local scroll = scrollParent or _G.SYNCE_SCROLL
    
    if not scroll then
        warn("[ERROR] Scroll parent not found!")
        return nil
    end
    
    print("[DEBUG] Creating card:", item.Name, "Category:", currentCat.Name, "AssetType:", currentCat.AssetType)
    ...
    -- FIXED: Preview hanya untuk emotes (AssetType 61)
    if currentCat.AssetType == 61 and Settings["Preview"] then
        local viewport, dummy = createViewport(
            UDim2.fromScale(1, 0.55),
            UDim2.fromScale(0, 0),
            card
        )
        -- FIXED: Use safe function
        playAnimation(dummy, safeGetAnimId(thumbId))
    else
        -- Image untuk semua category termasuk clothing
    ...
    actionBtn.Size = UDim2.new(0.45, -scale("X", 5), 0, scale("Y", 28))
    actionBtn.Position = UDim2.new(0, scale("X", 5), 1, -scale("Y", 33))
    actionBtn.Font = Enum.Font.GothamBold
    actionBtn.TextSize = scale("Y", 12)
    actionBtn.TextColor3 = Color3.new(1, 1, 1)
    actionBtn.Parent = card
    createCorner(actionBtn, 10)
    
    -- FIXED: Proper category detection berdasarkan AssetType
    if currentCat.AssetType == 61 then
        -- EMOTES
        actionBtn.BackgroundColor3 = Color3.fromRGB(80, 200, 120)
        actionBtn.Text = "▶ Play"
        addHoverEffect(actionBtn, Color3.fromRGB(100, 220, 140), Color3.fromRGB(80, 200, 120))
        
        actionBtn.MouseButton1Click:Connect(function()
            print("[DEBUG] Playing emote:", thumbId)
            LoadTrack(thumbId)
        end)
    else
        ...
        actionBtn.Text = "👔 Wear"
        addHoverEffect(actionBtn, Color3.fromRGB(120, 170, 255), Color3.fromRGB(100, 150, 255))
        
        actionBtn.MouseButton1Click:Connect(function()
            print("[DEBUG] Equipping item:", thumbId, "Type:", currentCat.AssetType)
            
            -- FIXED: Better wear system
            local success, errorMsg = pcall(function()
                -- Get fresh description
                local humanoidDesc = humanoid:GetAppliedDescription()
                
                -- Shirts
                if currentCat.AssetType == 11 then
                    humanoidDesc.Shirt = tonumber(thumbId)
                    print("[WEAR] Setting Shirt:", thumbId)
                -- Pants
                elseif currentCat.AssetType == 12 then
                    humanoidDesc.Pants = tonumber(thumbId)
                    print("[WEAR] Setting Pants:", thumbId)
                -- T-Shirts
                elseif currentCat.AssetType == 2 then
                    humanoidDesc.GraphicTShirt = tonumber(thumbId)
                    print("[WEAR] Setting TShirt:", thumbId)
                -- Faces
                elseif currentCat.AssetType == 18 then
                    humanoidDesc.Face = tonumber(thumbId)
                    print("[WEAR] Setting Face:", thumbId)
                -- Accessories (Hats, Hair, Back, Face Acc)
                elseif currentCat.AssetType == 8 or currentCat.AssetType == 41 or 
                       currentCat.AssetType == 42 or currentCat.AssetType == 44 then
                    
                    -- Convert to proper format
                    local accId = tonumber(thumbId)
                    if not accId then
                        warn("[WEAR] Invalid accessory ID")
                        return
                    end
                    
                    -- Check if already equipped
                    local alreadyHas = false
                    for _, existingId in ipairs(humanoidDesc:GetAccessories(true)) do
                        if existingId == accId then
                            alreadyHas = true
                            break
                        end
                    end
                    ...
                    if not alreadyHas then
                        humanoidDesc:AddAccessory(accId, Enum.AccessoryType.Hat)
                        print("[WEAR] Adding accessory:", accId)
                    else
                        print("[WEAR] Accessory already equipped")
                    end
                end
                
                -- Apply changes
                humanoid:ApplyDescription(humanoidDesc)
                print("[WEAR] Description applied successfully")
            end)
            ...
            if success then
                ...
                warn("[WEAR] Failed:", errorMsg)
                actionBtn.Text = "❌ Failed"
                actionBtn.BackgroundColor3 = Color3.fromRGB(220, 80, 80)
                task.wait(1.5)
                actionBtn.Text = "👔 Wear"
                actionBtn.BackgroundColor3 = Color3.fromRGB(100, 150, 255)
            end
    ...
    saveBtn.MouseButton1Click:Connect(function()
        ...
        if not alreadySaved then
            table.insert(savedEmotes, {
                ...
                AnimationId = currentCat.AssetType == 61 and ("rbxassetid://" .. safeGetAnimId(thumbId)) or nil,
                Category = currentCat.Name,
                AssetType = currentCat.AssetType,
                Favorite = false
            })
            saveEmotesToData()
    ...
    print("[DEBUG] Card created successfully")
    return card
end
...
-- ========================================
-- SYNCE CATALOG - MULTI CATEGORY (PART 4B/5 - FINAL) - FULLY FIXED
-- FIXED: Category Dropdown Visibility + Wear System
-- ========================================

-- PASTE THIS AFTER PART 4A

-- CRITICAL: Declare scroll FIRST before any functions use it
...
-- Set to global so createCard can access
_G.SYNCE_SCROLL = scroll
...
for i, category in ipairs(CATEGORIES) do
    local catItem = Instance.new("TextButton")
    catItem.Size = UDim2.new(1, 0, 0, scale("Y", 32))
    catItem.BackgroundColor3 = Color3.fromRGB(40, 40, 50)
    catItem.BackgroundTransparency = 0  -- FIXED: Make visible
    catItem.Text = category.Name
    catItem.Font = Enum.Font.GothamBold
    catItem.TextSize = scale("Y", 12)
    catItem.TextColor3 = Color3.new(1, 1, 1)
    catItem.TextXAlignment = Enum.TextXAlignment.Left
    catItem.BorderSizePixel = 0
    catItem.ZIndex = 11  -- FIXED: Above dropdown
    catItem.Parent = categoryDropdown
    createCorner(catItem, 8)
    
    local padding = Instance.new("UIPadding", catItem)
    padding.PaddingLeft = UDim.new(0, scale("X", 10))
    
    addHoverEffect(catItem, Color3.fromRGB(60, 60, 70), Color3.fromRGB(40, 40, 50))
    
    catItem.MouseButton1Click:Connect(function()
        ...
        print("[CATEGORY] Switched to:", category.Name)
        doNewSearch("")
    end)
    
    print("[DROPDOWN] Created button:", category.Name)
end

-- Update dropdown size
categoryDropdown.CanvasSize = UDim2.new(0, 0, 0, categoryLayout.AbsoluteContentSize.Y + 8)
print("[DROPDOWN] Canvas size set:", categoryLayout.AbsoluteContentSize.Y)

categoryBtn.MouseButton1Click:Connect(function()
    categoryDropdown.Visible = not categoryDropdown.Visible
    print("[DROPDOWN] Visibility:", categoryDropdown.Visible)
end)

-- FIXED: Better click outside detection
UserInputService.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        if categoryDropdown.Visible then
            local mousePos = UserInputService:GetMouseLocation()
            local dropPos = categoryDropdown.AbsolutePosition
            local dropSize = categoryDropdown.AbsoluteSize
            local btnPos = categoryBtn.AbsolutePosition
            local btnSize = categoryBtn.AbsoluteSize
            
            -- Check if clicked outside both button and dropdown
            local clickedButton = mousePos.X >= btnPos.X and mousePos.X <= btnPos.X + btnSize.X and
                                 mousePos.Y >= btnPos.Y and mousePos.Y <= btnPos.Y + btnSize.Y
            
            local clickedDropdown = mousePos.X >= dropPos.X and mousePos.X <= dropPos.X + dropSize.X and
                                   mousePos.Y >= dropPos.Y and mousePos.Y <= dropPos.Y + dropSize.Y
            
            if not clickedButton and not clickedDropdown then
                categoryDropdown.Visible = false
            end
        end
    end
...
local function showPage()
    print("[DEBUG] ===== SHOWPAGE START =====")
    print("[DEBUG] Category:", getCurrentCategory().Name)
    ...
    if start_index > #ITEM_CACHE and NEXT_API_CURSOR then
        ...
        local success = GetCatalogDataFromWeb()
        if not success then
            pageBox.Text = "Error"
    ...
    print("[DEBUG] Creating cards from", start_index, "to", math.min(end_index, #ITEM_CACHE))
    ...
    print("[DEBUG] Cards created:", cardsCreated)

    scroll.CanvasSize = UDim2.new(0, 0, 0, layout.AbsoluteContentSize.Y + 8)
    emptyLabel.Visible = (#ITEM_CACHE == 0)
    
    print("[DEBUG] ===== SHOWPAGE END =====")
end
...
sortBtn.MouseButton1Click:Connect(function()
    ...
    sortBtn.Text = "📊 " .. CURRENT_SORT_OPTION:sub(1, 3)
...
-- ========================================
-- FIXED: createSavedCard Function
-- ========================================
...
local function createSavedCard(item)
    ...
    categoryTag.Size = UDim2.new(0, scale("X", 60), 0, scale("Y", 18))
    categoryTag.Position = UDim2.new(0, scale("X", 8), 0, scale("Y", 8))
    categoryTag.BackgroundColor3 = Color3.fromRGB(80, 60, 180)
    categoryTag.Text = item.Category and item.Category:match("%S+") or "Item"
    categoryTag.Font = Enum.Font.GothamBold
    categoryTag.TextSize = scale("Y", 10)
    categoryTag.TextColor3 = Color3.new(1, 1, 1)
    categoryTag.Parent = card
    createCorner(categoryTag, 6)
    ...
    -- FIXED: useBtn with proper type detection
    local useBtn = Instance.new("TextButton")
    useBtn.Size = UDim2.new(0.45, -scale("X", 5), 0, scale("Y", 28))
    useBtn.Position = UDim2.new(0, scale("X", 5), 1, -scale("Y", 33))
    useBtn.Font = Enum.Font.GothamBold
    useBtn.TextSize = scale("Y", 12)
    useBtn.TextColor3 = Color3.new(1, 1, 1)
    useBtn.Parent = card
    createCorner(useBtn, 10)

    -- Check item type
    if item.AssetType == 61 or not item.AssetType then
        -- EMOTE
        useBtn.BackgroundColor3 = Color3.fromRGB(80, 200, 120)
        useBtn.Text = "▶ Play"
        addHoverEffect(useBtn, Color3.fromRGB(100, 220, 140), Color3.fromRGB(80, 200, 120))
        ...
        -- CLOTHING
        useBtn.BackgroundColor3 = Color3.fromRGB(100, 150, 255)
        useBtn.Text = "👔 Wear"
        addHoverEffect(useBtn, Color3.fromRGB(120, 170, 255), Color3.fromRGB(100, 150, 255))
        
        useBtn.MouseButton1Click:Connect(function()
            local success = pcall(function()
                local humanoidDesc = humanoid:GetAppliedDescription()
                
                if item.AssetType == 11 then
                    humanoidDesc.Shirt = tonumber(item.Id)
                elseif item.AssetType == 12 then
                    humanoidDesc.Pants = tonumber(item.Id)
                elseif item.AssetType == 2 then
                    humanoidDesc.GraphicTShirt = tonumber(item.Id)
                elseif item.AssetType == 18 then
                    humanoidDesc.Face = tonumber(item.Id)
                else
                    humanoidDesc:AddAccessory(tonumber(item.Id), Enum.AccessoryType.Hat)
                end
...
print("[INIT] Calling initial load...")
doNewSearch("")
...
local function toggleGui()
    ...
    if gui.Enabled then
        ...
        createSmoothTween(mainContainer, {Size = UDim2.new(0, scale("X", 650), 0, scale("Y", 450))}, 0.4)
    end
end
...
print("✨ Synce Catalog ULTIMATE - Successfully Loaded!")
print("🛍️ Press 'G' to toggle | All bugs fixed!")
