...
-- ========================================
-- SYNCE CATALOG - MULTI CATEGORY (PART 4A/5) - FINAL FIX
-- Fixed: AddAccessory error + proper clothing system
-- ========================================
...
-- ========================================
-- FIXED: Proper InsertService usage for accessories
-- ========================================
local InsertService = game:GetService("InsertService")

local function equipAccessory(assetId)
    local success, result = pcall(function()
        -- Load model dari catalog
        local model = InsertService:LoadAsset(tonumber(assetId))
        if not model then
            warn("[WEAR] Failed to load asset:", assetId)
            return false
        end
        
        -- Cari accessory di model
        local accessory = model:FindFirstChildOfClass("Accessory")
        if not accessory then
            warn("[WEAR] No accessory found in model")
            model:Destroy()
            return false
        end
        
        -- Remove existing accessories of same type (optional)
        ...
        print("[WEAR] Accessory equipped:", assetId)
        return true
    end)
    
    if not success then
        warn("[WEAR] Accessory equip failed:", result)
        return false
    end
...
-- ========================================
-- FIXED: createCard Function with proper equip
-- ========================================
...
local function createCard(item, scrollParent)
    ...
    if currentCat.AssetType == 61 then
        ...
        actionBtn.MouseButton1Click:Connect(function()
            ...
            -- FIXED: Proper equip logic
            if currentCat.AssetType == 8 or currentCat.AssetType == 41 or 
               currentCat.AssetType == 42 or currentCat.AssetType == 44 then
                -- ACCESSORIES - Use InsertService
                success = equipAccessory(thumbId)
            else
                -- CLOTHING - Use HumanoidDescription
                success = pcall(function()
                    ...
                    print("[WEAR] Description applied")
                end)
            end
...
-- ========================================
-- SYNCE CATALOG - MULTI CATEGORY (PART 4B/5 - FINAL) - COMPLETE FIX
-- Fixed: Saved items equip system
-- ========================================

-- PASTE THIS AFTER PART 4A

-- CRITICAL: Declare scroll FIRST
...
for i, category in ipairs(CATEGORIES) do
    ...
    catItem.BackgroundTransparency = 0
...
-- ========================================
-- FIXED: createSavedCard with proper equip
-- ========================================
...
local function createSavedCard(item)
    ...
    if item.AssetType == 61 or not item.AssetType then
        ...
        useBtn.MouseButton1Click:Connect(function()
            ...
            -- FIXED: Same logic as catalog cards
            if item.AssetType == 8 or item.AssetType == 41 or 
               item.AssetType == 42 or item.AssetType == 44 then
                -- ACCESSORIES
                success = equipAccessory(item.Id)
            else
                -- CLOTHING
                success = pcall(function()
                    local humanoidDesc = humanoid:GetAppliedDescription()
...
print("🛍️ Press 'G' to toggle | Full clothing support!")
