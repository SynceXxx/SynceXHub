...
function TabContent.CreateTabs(Window, Reg, WindUI, SynceHubConfig, SmartLoadConfig, BaseFolder, ElementRegistry)
            ...
            if state then
                ...
                if success then
                    ...
                    WindUI:Notify({ 
                        ...
                        Content = "Press G to toggle visibility",
                        Duration = 3, 
                        Icon = "eye-off" 
                    })
                else
                    WindUI:Notify({ 
                        Title = "Failed", 
                        Content = "Could not start invisible mode",
                        Duration = 3, 
                        Icon = "x" 
                    })
                end
    ...
    visibility:Keybind({
        Title = "Quick Toggle Key",
        Desc = "Hotkey untuk toggle invisibility saat aktif",
        Value = "G",
        Callback = function(key)
            ...
            invisKeybindConnection = UserInputService.InputBegan:Connect(function(input, processed)
                if processed then return end
                
                if input.KeyCode == Enum.KeyCode[key] and InvisSystem.isActive then
                    local currentlyVisible = InvisSystem.cachedParts[1] and InvisSystem.cachedParts[1].Transparency == 0
                    local newState = not currentlyVisible
                    
                    InvisSystem:Toggle(newState)
                    
                    WindUI:Notify({ 
                        Title = "Quick Toggle", 
                        Content = newState and "Semi-Invisible" or "Visible",
                        Duration = 1, 
                        Icon = newState and "eye-off" or "eye" 
                    })
                end
            end)
    ...
    visibility:Paragraph({
        Title = "How to Use",
        Desc = "1. Toggle ON to enable invisible mode\n2. Use hotkey (default: G) for quick visibility toggle\n3. Toggle OFF to disable completely\n\nNote: Semi-invisible means other players may still see you slightly.",
    })
