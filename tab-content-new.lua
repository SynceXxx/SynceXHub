...
function TabContent.CreateTabs(Window, Reg, WindUI, SynceHubConfig, SmartLoadConfig, BaseFolder, ElementRegistry)
    ...
    Reg("invis", other:Toggle({
        Title = "Invisible Character",
        Desc = "Make your character invisible (Press G to toggle)",
        Value = false,
        Callback = function(state)
            if state then
    ...
    -- Setup Quick Toggle Keybind (G Key)
    ...
    invisKeybindConnection = UserInputService.InputBegan:Connect(function(input, processed)
        ...
        if input.KeyCode == Enum.KeyCode.G and InvisSystem.isActive then
