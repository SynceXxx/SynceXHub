...
function FeatureModule.Init(Window, Reg, WindUI, LocalPlayer)
    ...
    function WalkFlingSystem.Toggle()
        ...
        if WalkFlingSystem.Active then
            -- Disable humanoid states to prevent ragdoll
            humanoid:SetStateEnabled(Enum.HumanoidStateType.FallingDown, false)
            humanoid:SetStateEnabled(Enum.HumanoidStateType.Ragdoll, false)
            
            -- Make character parts massless and no collide to prevent self-fling
            ...
            flingPart.CustomPhysicalProperties = PhysicalProperties.new(100, 0.3, 0, 100, 100)
            flingPart.Parent = workspace
            
            -- Add spinning velocity
            ...
            -- Keep fling part attached to character while walking
            WalkFlingSystem.Connections.update = _G.SynceHub.RunService.Heartbeat:Connect(function()
            ...
            -- Cleanup
            if WalkFlingSystem.Connections.update then
                WalkFlingSystem.Connections.update:Disconnect()
                WalkFlingSystem.Connections.update = nil
            end
    ...
    function WalkFlingSystem.SetPower(power)
        if tonumber(power) then
            ...
            -- Update current fling if active
            if WalkFlingSystem.Active then
                local flingPart = workspace:FindFirstChild("WalkFlingPart")
    ...
    _G.SynceHub.AntiFlingSystem = AntiFlingSystem
    
    AntiFlingSystem.Active = false
    AntiFlingSystem.Connections = {}
    
    function AntiFlingSystem.Toggle()
        AntiFlingSystem.Active = not AntiFlingSystem.Active
        ...
        if AntiFlingSystem.Active then
            -- Create anti-fling connection
            AntiFlingSystem.Connections.velocity = _G.SynceHub.RunService.Heartbeat:Connect(function()
                local char = LocalPlayer.Character
                if not char then return end
                ...
                -- Reset velocity if too high (being flung)
                ...
                -- Remove any suspicious body movers
                for _, obj in pairs(root:GetChildren()) do
                    if obj:IsA("BodyAngularVelocity") or obj:IsA("BodyVelocity") then
                        if obj.Name ~= "VelocityHandler" and obj.Name ~= "GyroHandler" then
                            obj:Destroy()
                        end
                    end
                end
            ...
            -- Disconnect anti-fling
            if AntiFlingSystem.Connections.velocity then
                AntiFlingSystem.Connections.velocity:Disconnect()
                AntiFlingSystem.Connections.velocity = nil
            end
        end
        
        return AntiFlingSystem.Active
    end
    
    function AntiFlingSystem.Cleanup()
        AntiFlingSystem.Active = false
        
        for _, conn in pairs(AntiFlingSystem.Connections) do
            if conn and conn.Connected then
                conn:Disconnect()
            end
        end
