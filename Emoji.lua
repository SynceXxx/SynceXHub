-- ========================================
-- SYNCE EMOTES - FIXED VERSION (PART 1/5)
-- Core Setup, Services, and Utilities
-- ========================================

-- Screen Utility (FIXED - Only one definition)
...
-- Settings (FIXED - Consistent naming)
local Settings = {}
...
Settings["Weight"] = 1
Settings["Speed"] = 1
Settings["Allow Invisible"] = true -- FIXED: Removed extra spaces
...
local function createStroke(parent, color, thickness)
    ...
    stroke.Thickness = thickness or 0.3
    stroke.Transparency = 0.8
    stroke.Parent = parent
    return stroke
end

-- FIXED: More robust hover effect with debounce
local function addHoverEffect(button, hoverColor, normalColor)
    ...
    button.MouseLeave:Connect(function()
        ...
        createSmoothTween(button, {Size = originalSize}, 0.1)
    end)
    
    button.MouseButton1Down:Connect(function()
        if isHovering then
            createSmoothTween(button, {Size = originalSize - UDim2.new(0, scale("X", 2), 0, scale("Y", 2))}, 0.1)
        end
    end)
...
createStroke(mainContainer, Color3.fromRGB(70, 70, 85), 0.8)

-- FIXED: Shadow with proper ZIndex
...
shadowFrame.ZIndex = 0 -- FIXED: Changed from -1 to 0
...
print("✅ Part 1 Loaded - Core Setup Complete")

-- ========================================
-- SYNCE EMOTES - FIXED VERSION (PART 2/5)
-- Header, Tabs, and Frame Setup
-- ========================================
...
titleBar.Size = UDim2.new(1, 0, 0, scale("Y", 55))
titleBar.BackgroundColor3 = Color3.fromRGB(25, 25, 32)
titleBar.Parent = mainContainer
createCorner(titleBar, 16)

local titleIcon = Instance.new("TextLabel")
titleIcon.Size = UDim2.new(0, scale("X", 45), 0, scale("Y", 45))
titleIcon.Position = UDim2.new(0, scale("X", 10), 0.5, -scale("Y", 22.5))
...
title.Size = UDim2.new(0, scale("X", 250), 0, scale("Y", 35))
title.Position = UDim2.new(0, scale("X", 65), 0.5, -scale("Y", 17.5))
...
titleAccent.Size = UDim2.new(0, scale("X", 100), 0, scale("Y", 18))
titleAccent.Position = UDim2.new(0, scale("X", 65), 0.5, scale("Y", 10))
...
titleAccent.TextSize = scale("Y", 13)
titleAccent.TextXAlignment = Enum.TextXAlignment.Left
titleAccent.Parent = titleBar

-- Minimize Button
local minimizeBtn = Instance.new("TextButton")
minimizeBtn.Size = UDim2.new(0, scale("X", 35), 0, scale("Y", 35))
minimizeBtn.Position = UDim2.new(1, -scale("X", 85), 0.5, -scale("Y", 17.5))
minimizeBtn.BackgroundColor3 = Color3.fromRGB(60, 60, 75)
minimizeBtn.Text = "−"
minimizeBtn.Font = Enum.Font.GothamBold
minimizeBtn.TextSize = scale("Y", 20)
minimizeBtn.TextColor3 = Color3.fromRGB(200, 200, 210)
minimizeBtn.Parent = titleBar
createCorner(minimizeBtn, 8)
addHoverEffect(minimizeBtn, Color3.fromRGB(80, 80, 95), Color3.fromRGB(60, 60, 75))

-- Close Button
local closeBtn = Instance.new("TextButton")
closeBtn.Size = UDim2.new(0, scale("X", 35), 0, scale("Y", 35))
closeBtn.Position = UDim2.new(1, -scale("X", 45), 0.5, -scale("Y", 17.5))
closeBtn.BackgroundColor3 = Color3.fromRGB(220, 60, 60)
closeBtn.Text = "×"
closeBtn.Font = Enum.Font.GothamBold
closeBtn.TextSize = scale("Y", 24)
closeBtn.TextColor3 = Color3.new(1, 1, 1)
closeBtn.Parent = titleBar
createCorner(closeBtn, 8)
addHoverEffect(closeBtn, Color3.fromRGB(240, 80, 80), Color3.fromRGB(220, 60, 60))
...
tabContainer.Position = UDim2.new(0, scale("X", 10), 0, scale("Y", 65))
tabContainer.BackgroundTransparency = 1
tabContainer.Parent = mainContainer

local catalogTabBtn = Instance.new("TextButton")
catalogTabBtn.Size = UDim2.new(0.5, -scale("X", 2), 1, 0)
...
savedTabBtn.Size = UDim2.new(0.5, -scale("X", 2), 1, 0)
savedTabBtn.Position = UDim2.new(0.5, scale("X", 2), 0, 0)
...
divider.Size = UDim2.new(0, scale("X", 2), 1, -scale("Y", 115))
divider.Position = UDim2.new(0.62, -scale("X", 1), 0, scale("Y", 115))
...
catalogFrame.Size = UDim2.new(0.62, -scale("X", 15), 1, -scale("Y", 120))
catalogFrame.Position = UDim2.new(0, scale("X", 10), 0, scale("Y", 115))
...
savedFrame.Size = UDim2.new(0.62, -scale("X", 15), 1, -scale("Y", 120))
savedFrame.Position = UDim2.new(0, scale("X", 10), 0, scale("Y", 115))
...
settingsFrame.Size = UDim2.new(0.38, -scale("X", 15), 1, -scale("Y", 120))
settingsFrame.Position = UDim2.new(0.62, scale("X", 5), 0, scale("Y", 115))
...
print("✅ Part 2 Loaded - UI Frames Complete")

-- ========================================
-- SYNCE EMOTES - FIXED VERSION (PART 3/5)
-- Sliders, Toggles, and Collision System
-- ========================================
...
createToggle("Allow Invisible") -- FIXED: No extra spaces
...
local function restoreCollisionStates()
    ...
    originalCollisionStates = {} -- FIXED: Clear after restoring
end
...
player.CharacterAdded:Connect(function(newCharacter)
    restoreCollisionStates() -- FIXED: Clean up old character data
    character = newCharacter
    humanoid = newCharacter:WaitForChild("Humanoid")
    saveCollisionStates()
...
print("✅ Part 3 Loaded - Settings & Collision System Complete")

-- ========================================
-- SYNCE EMOTES - FIXED VERSION (PART 4/5)
-- Catalog System with Improved Error Handling
-- ========================================
...
-- FIXED: Better error handling and retry mechanism
local function GetEmoteDataFromWeb()
    -- Check if request function exists
    if not request then
        warn("[ERROR] Request function not available in this executor!")
        return false
    end
    ...
    if not result or not result.Success then
        warn("[ERROR] Request not successful:", result and result.StatusCode or "Unknown")
        return false
    end
    ...
    if not success then
        warn("[ERROR] JSON Parse failed:", tostring(data_table))
        return false
    end
    
    if not data_table or not data_table.data then
        warn("[ERROR] No data field in response")
        return false
    end
...
layout.CellSize = UDim2.new(0, scale("X", 125), 0, scale("Y", 165))
...
local function createCard(item)
    ...
    card.Size = UDim2.new(0, scale("X", 125), 0, scale("Y", 165))
    ...
    img.Size = UDim2.new(1, -scale("X", 10), 0, scale("Y", 70))
    ...
    name.Position = UDim2.new(0, scale("X", 5), 0, scale("Y", 80))
...
-- FIXED: Better canvas size update
local function showPage()
    ...
    if start_index > #ITEM_CACHE and NEXT_API_CURSOR then
        ...
        if not success then
            pageBox.Text = "Error"
            emptyLabel.Text = "❌ Failed to load data"
    ...
    -- FIXED: Update canvas size after all items loaded
    task.wait(0.1)
...
local function doNewSearch(keyword)
    ...
    if success then
        ...
        pageBox.Text = "Failed"
        emptyLabel.Text = "❌ Search failed. Check console for details."
        emptyLabel.Visible = true
    end
end
...
print("✅ Part 4 Loaded - Catalog System Complete")

-- ========================================
-- SYNCE EMOTES - FIXED VERSION (PART 5/5 - FINAL)
-- Saved Tab and Toggle Button with Debounce
-- ========================================
...
screonGui.Name = "SynceToggleButton"
...
btn.Image = "rbxassetid://114915707934715"
...
btn.MouseEnter:Connect(function()
    createSmoothTween(btn, {Size = UDim2.new(0, 55, 0, 55)}, 0.2)
    createSmoothTween(btn, {ImageTransparency = 0}, 0.2)
end)

btn.MouseLeave:Connect(function()
    createSmoothTween(btn, {Size = UDim2.new(0, 50, 0, 50)}, 0.2)
    createSmoothTween(btn, {ImageTransparency = 0}, 0.2)
end)
...
-- FIXED: Improved debounce system
...
local function toggleGui()
    ...
    if gui.Enabled then
        ...
        createSmoothTween(btn, {Rotation = 360}, 0.3)
        ...
        createSmoothTween(btn, {Rotation = -360}, 0.4)
...
-- Get buttons from global storage
local closeBtn = _G.SynceButtons.Close
local minimizeBtn = _G.SynceButtons.Minimize
...
minimizeBtn.MouseButton1Click:Connect(function()
    ...
    if not isMinimized then
        ...
        createSmoothTween(mainContainer, {
            Size = UDim2.new(mainContainer.Size.X.Scale, mainContainer.Size.X.Offset, 0, scale("Y", 55))
        }, 0.3)
        minimizeBtn.Text = "□"
        isMinimized = true
    else
...
print("✨ Synce Emotes UI - FIXED VERSION Successfully Loaded!")
print("📌 Press 'G' or click toggle button to open")
print("🔧 All bugs fixed:")
print("  ✅ Screen variable conflict resolved")
print("  ✅ Collision system cleanup improved")
print("  ✅ Error handling added for API requests")
print("  ✅ Canvas size updates fixed")
print("  ✅ Toggle debounce improved")
print("  ✅ Setting names standardized")
print("  ✅ ZIndex values corrected")
