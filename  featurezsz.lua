...
function FeatureModule.Init(Window, Reg, WindUI, LocalPlayer)
    ...
    -- ============================================================
    local FlingSystem = {}
    _G.SynceHub.FlingSystem = FlingSystem
    
    FlingSystem.Active = false
    FlingSystem.Power = 500
    FlingSystem.Connections = {}
    FlingSystem.OriginalVelocity = nil
    
    function FlingSystem.Toggle()
        ...
        if FlingSystem.Active then
            ...
            if hrp:FindFirstChild("BodyVelocity") then
                FlingSystem.OriginalVelocity = hrp.BodyVelocity
            end
            ...
            bv.P = 1000000
            bv.AngularVelocity = Vector3.new(0, FlingSystem.Power, 0)
            
            -- Disable humanoid state to prevent death
            local humanoid = character:FindFirstChildOfClass("Humanoid")
            if humanoid then
        ...
        return FlingSystem.Active
    end
    
    function FlingSystem.SetPower(power)
        if tonumber(power) then
            FlingSystem.Power = tonumber(power)
            ...
            if FlingSystem.Active then
                ...
                if character then
                    ...
                    if hrp and hrp:FindFirstChild("FlingVelocity") then
                        hrp.FlingVelocity.AngularVelocity = Vector3.new(0, FlingSystem.Power, 0)
                    end
                end
            end
        end
    ...
    function AntiFlingSystem.Toggle()
        ...
        if AntiFlingSystem.Active then
            ...
            AntiFlingSystem.Connections.velocity = _G.SynceHub.RunService.Heartbeat:Connect(function()
                ...
                -- Remove any fling body movers from other players
                for _, obj in pairs(root:GetChildren()) do
                    if obj:IsA("BodyAngularVelocity") and obj.Name ~= "FlingVelocity" then
                        obj:Destroy()
                    end
                end
            end)
