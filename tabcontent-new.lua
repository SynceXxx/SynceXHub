...
-- VISIBILITY SECTION
-- ============================================================
local visibility = player:Section({
    Title = "Visibility",
    Icon = "eye",
    TextSize = 19,
})
...
-- ============================================================
-- INVISIBLE CHARACTER SYSTEM (DEBUG VERSION)
-- ============================================================
local InvisSystem = _G.SynceHub.InvisibleSystem
local invisKeybindConnection = nil

-- CHECK: Apakah system ada?
if not InvisSystem then
    warn("[ERROR] InvisibleSystem not found in _G.SynceHub!")
    visibility:Paragraph({
        Title = "⚠️ System Error",
        Desc = "InvisibleSystem failed to load. Check console for errors.",
    })
else
    print("[DEBUG] InvisibleSystem found:", InvisSystem)
end

Reg("invis", visibility:Toggle({
    Title = "Invisible Character",
    Desc = "Make your character invisible to others",
    Value = false,
    Callback = function(state)
        print("[DEBUG] Toggle callback triggered. State:", state)
        print("[DEBUG] InvisSystem exists:", InvisSystem ~= nil)
        
        if state then
            print("[DEBUG] Attempting to start invisible mode...")
            ...
            print("[DEBUG] Humanoid:", hum)
            print("[DEBUG] HRP:", hrp)
            
            if not hum or not hrp then
                WindUI:Notify({ 
                    Title = "Error", 
                    Content = "Missing Humanoid or HumanoidRootPart!",
                    Duration = 3, 
                    Icon = "x" 
                })
                return
            ...
            local success = InvisSystem:Start()
            print("[DEBUG] Start result:", success)
            print("[DEBUG] Cached parts count:", #InvisSystem.cachedParts)
            
            if success then
                InvisSystem:Toggle(true)
                print("[DEBUG] Toggle(true) called")
                
                -- Check if parts are actually transparent
                local firstPart = InvisSystem.cachedParts[1]
                if firstPart then
                    print("[DEBUG] First part transparency:", firstPart.Transparency)
                end
                
                WindUI:Notify({ 
                    Title = "Invisible Mode ON", 
                    Content = string.format("Parts cached: %d | Press G to toggle", #InvisSystem.cachedParts),
                    Duration = 3, 
                    Icon = "eye-off" 
                })
            else
                WindUI:Notify({ 
                    Title = "Failed", 
                    Content = "Could not start invisible mode. Check console!",
                    Duration = 3, 
                    Icon = "x" 
                })
            end
        else
            print("[DEBUG] Stopping invisible mode...")
            InvisSystem:Stop()
