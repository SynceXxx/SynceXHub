...
-- Catalog System - FIXED WITH DEBUG
-- ========================================
...
-- FIXED: Tambah debug logging
local function GetEmoteDataFromWeb()
    print("[DEBUG] Starting API request...")
    print("[DEBUG] Current SEARCH_TEXT:", CURRENT_SEARCH_TEXT)
    print("[DEBUG] Current SORT_OPTION:", CURRENT_SORT_OPTION)
    ...
    if not result or not result.Success then
        warn("[ERROR] Request not successful. Status:", result and result.StatusCode or "unknown")
        return false
    end

    response = result.Body
    print("[DEBUG] Got response body, length:", #response)
    ...
    print("[DEBUG] Parsed", #data_table.data, "items from API")
    ...
    print("[SUCCESS] Total cached items:", #ITEM_CACHE)
    print("[DEBUG] Next cursor:", NEXT_API_CURSOR or "none")
...
local function createCard(item)
    print("[DEBUG] Creating card for:", item.Name, "ID:", item.Id)
    ...
    playBtn.MouseButton1Click:Connect(function()
        print("[DEBUG] Playing emote ID:", thumbId)
        LoadTrack(thumbId)
    end)
...
-- FIXED: Cards Parent ke Scroll yang Benar
-- ========================================

-- PASTE THIS AFTER PART 4A

-- FIXED: Declare scroll SEBELUM showPage dipanggil
...
-- FIXED: createCard function harus return dan parent ke scroll
local function createCard(item)
    print("[DEBUG] Creating card for:", item.Name, "ID:", item.Id)
    ...
    card.Size = UDim2.new(0, scale("X", 125), 0, scale("Y", 195))
    card.BackgroundColor3 = Color3.fromRGB(32, 32, 40)
    card.Parent = scroll -- CRITICAL FIX: Parent langsung ke scroll
    createCorner(card, 16)
...
local function showPage()
    print("[DEBUG] showPage called - Clearing existing cards")
    ...
    print("[DEBUG] Creating cards from index", start_index, "to", math.min(end_index, #ITEM_CACHE))
    ...
    print("[DEBUG] showPage complete. Total cards in scroll:", #scroll:GetChildren() - 2) -- -2 untuk layout dan emptyLabel
end
...
print("[DEBUG] Calling initial search...")
doNewSearch("")
