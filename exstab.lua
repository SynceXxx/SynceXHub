...
function TabContent.CreateTabs(Window, Reg, WindUI, SynceHubConfig, SmartLoadConfig, BaseFolder, ElementRegistry)
    ...
    -- FPS ULTRA BOOST (UPDATED)
    Reg("togfps", misc:Toggle({
        Title = "FPS Ultra Boost",
        Desc = "Extreme performance mode - Removes all effects",
        Value = false,
        Icon = "zap",
        Callback = function(state)
            local FPSBoostSystem = _G.SynceHub.FPSBoostSystem
            
            if state then
                FPSBoostSystem.ApplyBoost()
                
                WindUI:Notify({ 
                    Title = "FPS Boost ON", 
                    Content = "Maximum performance mode enabled!", 
                    Duration = 3, 
                    Icon = "zap" 
                })
            else
                FPSBoostSystem.RestoreSettings()
                
                WindUI:Notify({ 
                    Title = "FPS Boost OFF", 
                    Content = "Graphics restored. Rejoin for full restore.", 
                    Duration = 4, 
                    Icon = "rotate-ccw" 
                })
            end
