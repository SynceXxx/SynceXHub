...
function FeatureModule.Init(Window, Reg, WindUI, LocalPlayer)
    ...
    -- ============================================================
    -- FLING SYSTEM (IMPROVED - CHARACTER STAYS STILL)
    -- ============================================================
    local FlingSystem = {}
    ...
    function FlingSystem.Toggle()
        ...
        if FlingSystem.Active then
            -- Make character immune to physics
            ...
            -- Anchor HRP to keep character still
            hrp.Anchored = true
            
            -- Create invisible spinning part for fling
            ...
            flingPart.CustomPhysicalProperties = PhysicalProperties.new(100, 0, 0, 0, 0)
            flingPart.Parent = workspace
            
            -- Add spinning force to invisible part
            local bav = Instance.new("BodyAngularVelocity")
            bav.Name = "FlingVelocity"
            bav.Parent = flingPart
            bav.MaxTorque = Vector3.new(math.huge, math.huge, math.huge)
            bav.P = 1000000
            bav.AngularVelocity = Vector3.new(0, FlingSystem.Power, 0)
            
            -- Keep invisible part attached to character
            FlingSystem.Connections.update = _G.SynceHub.RunService.Heartbeat:Connect(function()
                if not FlingSystem.Active then return end
                ...
                -- Keep fling part at character position
            ...
            -- Cleanup
            if FlingSystem.Connections.update then
                FlingSystem.Connections.update:Disconnect()
                FlingSystem.Connections.update = nil
            end
            
            -- Remove fling part
            local flingPart = workspace:FindFirstChild("FlingPart")
            if flingPart then
                flingPart:Destroy()
            end
        ...
        if tonumber(power) then
            ...
            if FlingSystem.Active then
                ...
                if flingPart and flingPart:FindFirstChild("FlingVelocity") then
                    flingPart.FlingVelocity.AngularVelocity = Vector3.new(0, FlingSystem.Power, 0)
                end
            end
        end
    end
